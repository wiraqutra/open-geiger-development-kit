...
extern bool inMeasure ;
extern int MeasureCount  ;
extern int LastMeasureCount ;
...
int BtComm::read(int timeout)
{
    ...
    for( int i = timeout ; i != 0 ; i -- ) {
        ...
        if( inMeasure ) {
            if( MeasureCount != LastMeasureCount ) {
                //測定時、新しいデータが出たら文字列送出
                int NowGeigerCounter ;
                cli();//割り込み禁止
                LastMeasureCount = MeasureCount ;
                NowGeigerCounter = LastGeigerCounter ;
                sei();//割り込み許可
                Serial.println(NowGeigerCounter);
            }
        }
